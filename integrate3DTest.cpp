...
int integrate3DTest(float xp, float yp, float zp, int nRand, float lowerLimitX, float upperLimitX, float lowerLimitY, float upperLimitY, float lowerLimitZ, float upperLimitZ, float lowerLimitRho, float upperLimitRho){
  ...
  for(int i=0; i<nRand; i++){
    ...
    if(abs(fRandRho) <= abs(test1(xp,yp,zp,fRandX,fRandY,fRandZ))){
      nAccept++;
    }
